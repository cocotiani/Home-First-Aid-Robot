...
sss.connect(('192.168.43.150', 6666))   #链接刚刚绑定的ip和端口号
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:#手动操纵的意思
    ...
    class PilotCondition: #判断是否需要自动驾驶
        def run(self, mode):
            if mode == 'user':
                return False
            else:
    ...
    class DriveMode:
        def run(self, mode,
            ...
            #elif mode =='local_pilot'and msg==3:
            else':
    ...
    class PrintData:
        '''
         Show the real-time data while self-driving
        '''
        def run(self, pilot_angle, pilot_throttle):
            print("real-time angle:", pilot_angle)
            print("real-time throttle", pilot_throttle)


    V.add(PrintData(), inputs=['angle','throttle'], outputs=[])
